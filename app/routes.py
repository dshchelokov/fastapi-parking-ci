...
def parkings():
    if request.method == 'GET':
        ...
        return jsonify([{'id': p.id, 'address': p.address, 'opened': p.opened,
                         'count_places': p.count_places, 'count_available_places': p.count_available_places}
                        for p in parkings_list])
    elif request.method == 'POST':
        data = request.get_json()
        parking = Parking(address=data['address'], opened=data.get('opened', False),
...
def entry_parking():
    ...
    existing = ClientParking.query.filter_by(client_id=client.id, parking_id=parking.id).first()
    if existing:
...
def exit_parking():
    ...
    client_parking = ClientParking.query.filter_by(client_id=client.id, parking_id=parking.id).first_or_404()
